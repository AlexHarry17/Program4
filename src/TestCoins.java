...
//    }
    ...
    @Test
	void test_coin_option_size(){
    	testCoins = new Coins();
    	int expected = 3;
    	int [] coin_option = {1,5,7};

    	assertEquals(3, testCoins.getCoin_option_size(coin_option));
	}

	@Test
	void test_optimal_solution(){
		testCoins = new Coins();
		int change = 30;
		int [] coins = {1,5,10,25};
		int expected = 2;
		...
		assertEquals(expected,testCoins.get_optimal_solution(change));
...
//
//		// test correct coins return
//		assertEquals(expected,   testCoins.makeChange(coins, change));
	}
